...
class WebScraper {
  ...
  async scrapeAdmissions(admissionsUrl) {
    ...
    $('*').each((i, elem) => {
      ...
      deadlinePatterns.forEach(pattern => {
        ...
        if (matches) matches.forEach(match => data.deadlines.push(match.trim()));
      });
    });

    $('*:contains("requirement"), *:contains("requirements")').each((i, elem) => {
      const text = $(elem).text().trim();
      if (text.length > 20 && text.length < 500) data.requirements.push(text);
    });
  ...
  async scrapePrograms(programsUrl) {
    ...
    $('a, h2, h3, h4, .program, .degree, .major').each((i, elem) => {
      ...
      if (isProgram && text.length > 5 && text.length < 100) programs.add(text);
    });
  ...
  validateData(data, dataType) {
    if (!data) return false;
    switch (dataType) {
      case 'deadlines': return data.deadlines && data.deadlines.length > 0;
      case 'requirements': return data.requirements && data.requirements.length > 0;
      case 'programs': return Array.isArray(data) && data.length > 0;
