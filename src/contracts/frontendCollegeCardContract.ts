import { z } from 'zod';

export const FRONTEND_CANONICAL_RELATION = 'canonical.mv_college_cards' as const;

export const FRONTEND_COLLEGE_CARD_FIELDS = [
  'id',
  'canonical_name',
  'country_code',
  'state_region',
  'city',
  'website',
  'logo_url',
  'description',
  'institution_type',
  'popularity_score',
  'global_rank',
  'acceptance_rate',
  'test_optional',
  'sat_50',
  'act_50',
  'tuition_international',
  'cost_of_attendance',
  'avg_financial_aid',
  'merit_scholarship_flag',
  'need_blind_flag',
  'graduation_rate_4yr',
  'employment_rate',
  'median_start_salary',
  'metadata',
] as const;

export const FRONTEND_COLLEGE_CARD_REQUIRED_FIELDS = [
  'id',
  'canonical_name',
] as const;

export const FRONTEND_COLLEGE_CARD_ORDER_FIELDS = [
  'canonical_name',
  'acceptance_rate',
  'cost_of_attendance',
  'global_rank',
  'popularity_score',
] as const;

export const FRONTEND_COLLEGE_CARD_COLUMNS = FRONTEND_COLLEGE_CARD_FIELDS.join(', ');

export const FrontendCollegeCardSchema = z.object({
  id: z.union([z.string(), z.number()]),
  canonical_name: z.string().min(1, 'canonical_name is required'),
  country_code: z.string().nullable().default(null),
  state_region: z.string().nullable().default(null),
  city: z.string().nullable().default(null),
  website: z.string().nullable().default(null),
  logo_url: z.string().nullable().default(null),
  description: z.string().nullable().default(null),
  institution_type: z.string().nullable().default(null),
  popularity_score: z.number().nullable().default(0),
  global_rank: z.number().nullable().default(null),
  acceptance_rate: z.number().nullable().default(null),
  test_optional: z.boolean().nullable().default(null),
  sat_50: z.number().nullable().default(null),
  act_50: z.number().nullable().default(null),
  tuition_international: z.number().nullable().default(null),
  cost_of_attendance: z.number().nullable().default(null),
  avg_financial_aid: z.number().nullable().default(null),
  merit_scholarship_flag: z.boolean().nullable().default(null),
  need_blind_flag: z.boolean().nullable().default(null),
  graduation_rate_4yr: z.number().nullable().default(null),
  employment_rate: z.number().nullable().default(null),
  median_start_salary: z.number().nullable().default(null),
  metadata: z.record(z.unknown()).nullable().default({}),
});

export type FrontendCollegeCard = z.infer<typeof FrontendCollegeCardSchema>;

export const FRONTEND_COLLEGE_CARD_FALLBACKS: Omit<FrontendCollegeCard, 'id'> = {
  canonical_name: 'Unknown College',
  country_code: null,
  state_region: null,
  city: null,
  website: null,
  logo_url: null,
  description: null,
  institution_type: null,
  popularity_score: 0,
  global_rank: null,
  acceptance_rate: null,
  test_optional: null,
  sat_50: null,
  act_50: null,
  tuition_international: null,
  cost_of_attendance: null,
  avg_financial_aid: null,
  merit_scholarship_flag: null,
  need_blind_flag: null,
  graduation_rate_4yr: null,
  employment_rate: null,
  median_start_salary: null,
  metadata: {},
};

export function parseFrontendCollegeCardOrThrow(input: unknown): FrontendCollegeCard {
  const parsed = FrontendCollegeCardSchema.safeParse(input);
  if (!parsed.success) {
    throw new Error(`Frontend college card contract violation: ${parsed.error.issues.map((i) => `${i.path.join('.')}: ${i.message}`).join('; ')}`);
  }
  return parsed.data;
}

export function applyFrontendCollegeCardFallback(input: Partial<FrontendCollegeCard> & { id: string | number }): FrontendCollegeCard {
  return parseFrontendCollegeCardOrThrow({
    ...FRONTEND_COLLEGE_CARD_FALLBACKS,
    ...input,
  });
}
