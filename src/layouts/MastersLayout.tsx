...
const MastersLayout = () => {
  ...
  const navigation = [
    { name: 'Dashboard', href: '/masters', icon: Home },
    { name: 'Programs', href: '/masters/programs', icon: School },
    { name: 'Timeline', href: '/masters/timeline', icon: Clock },
    { name: 'Deadlines', href: '/masters/deadlines', icon: Calendar },
    { name: 'Funding', href: '/masters/funding', icon: DollarSign },
    { name: 'Applications', href: '/masters/applications', icon: FileText },
    { name: 'Settings', href: '/masters/settings', icon: SettingsIcon },
  ];
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-bg-primary)', color: 'var(--color-text-primary)', fontFamily: "'Inter', system-ui, sans-serif" }}>
        ...
        <div className="flex flex-col h-full">
          ...
          >
            <div>
              ...
              <p style={{ fontSize: 12, color: 'var(--color-text-secondary)', marginTop: 4 }}>Masters Track</p>
            </div>
            <button
              onClick={() => setSidebarOpen(false)}
          ...
          {/* Navigation */}
          <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto" aria-label="Masters navigation">
            {navigation.map((item) => (
              <NavLink
                key={item.name}
                to={item.href}
                end={item.href === '/masters'}
                onClick={() => setSidebarOpen(false)}
                className={({ isActive }) =>
      ...
      <nav
        ...
        role="navigation"
        aria-label="Mobile Masters navigation"
      >
