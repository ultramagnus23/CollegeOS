...
/* ─── Design tokens ──────────────────────────────────────────────────── */
import { h2r, S, GLOBAL, ACCENT_COLORS } from '../styles/designTokens';
const ACCENT  = ACCENT_COLORS.dashboard;
const ACCENT2 = ACCENT_COLORS.academics;
...
const SectionHead: React.FC<{ emoji: string; title: string; href?: string; linkLabel?: string }> = ({ emoji, title, href, linkLabel }) => (
  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: 16 }}>
    ...
    {href && linkLabel && (
      <Link to={href} style={{ fontSize: 12, color: h2r(ACCENT, 0.8), fontFamily: S.font, textDecoration: 'none', fontWeight: 600, display:'flex', alignItems:'center', gap:4 }}>
        {linkLabel} →
      </Link>
    )}
  </div>
...
const DeadlineRow: React.FC<{ deadline: any; getDaysUntil: (d: string) => string }> = ({ deadline, getDaysUntil }) => {
  ...
  return (
    <div style={{
      ...
      <div>
        <div style={{ fontSize: 14, fontWeight: 700, color: S.text, fontFamily: S.font, marginBottom: 2 }}>{deadline.college_name}</div>
    ...
    <>
      ...
      <div style={{ minHeight:'100vh', background:S.bg, display:'flex', alignItems:'center', justifyContent:'center' }}>
        <div style={{ textAlign:'center' }}>
          <div style={{ width:48, height:48, borderRadius:'50%', border:'3px solid rgba(255,255,255,0.08)', borderTopColor:ACCENT, animation:'spin 0.8s linear infinite', margin:'0 auto 16px' }} />
    ...
    <>
      ...
      <div style={{ minHeight:'100vh', background:S.bg, color:S.text, fontFamily:S.font }}>
        ...
        <div style={{
          ...
          background:`linear-gradient(135deg, ${h2r(ACCENT,0.12)} 0%, ${h2r(ACCENT2,0.06)} 50%, transparent 100%)`,
          borderBottom:`1px solid ${S.border}`,
          position:'relative', overflow:'hidden',
        }}>
          {/* Ambient orb */}
          <div style={{ position:'absolute', top:-100, right:-60, width:400, height:400, background:`radial-gradient(circle, ${h2r(ACCENT,0.1)} 0%, transparent 70%)`, borderRadius:'50%', pointerEvents:'none' }} />

          <div style={{ maxWidth:1280, margin:'0 auto', position:'relative', zIndex:1 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:28, flexWrap:'wrap', gap:16 }}>
              <div>
                <div style={{ fontSize:12, color:h2r(ACCENT,0.8), textTransform:'uppercase', letterSpacing:'0.12em', marginBottom:12, fontWeight:600 }}>Mission Control</div>
              ...
              <div style={{
                ...
                background:'rgba(255,255,255,0.07)', border:`1px solid ${h2r(ACCENT,0.3)}`,
                borderRadius:100, fontSize:13, fontWeight:600, color:h2r(ACCENT,0.9),
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))', gap:14 }}>
              <HeroStat icon="📝" label="Applications" value={stats.applications} accent={ACCENT} />
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:20, marginBottom:24 }}>
            ...
            <Card>
              ...
              {targetCountries.length > 0 ? targetCountries.map((c:string, i:number) => (
                <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 12px', background:h2r(ACCENT2,0.08), border:`1px solid ${h2r(ACCENT2,0.2)}`, borderRadius:8, marginBottom:8 }}>
                  <span style={{ fontSize:14, color:S.text, fontFamily:S.font }}>{c}</span>
                  <span style={{ fontSize:11, color:ACCENT2, fontWeight:600, fontFamily:S.font }}>Active</span>
                </div>
            ...
            <Card>
              ...
              {upcomingDeadlines.length === 0 ? (
                <div style={{ textAlign:'center', padding:'32px 0' }}>
                  ...
                  <Link to="/deadlines"><button style={{ marginTop:12, padding:'8px 18px', background:ACCENT, border:'none', borderRadius:10, color:'#fff', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:S.font }}>Add Deadline</button></Link>
                </div>
            ...
            <Card>
              ...
              {recentApplications.length === 0 ? (
                <div style={{ textAlign:'center', padding:'32px 0' }}>
                  ...
                  <Link to="/discover"><button style={{ marginTop:12, padding:'8px 18px', background:ACCENT, border:'none', borderRadius:10, color:'#fff', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:S.font }}>Browse Colleges</button></Link>
                </div>
                ...
                <div key={app.id} style={{
                  ...
                  onMouseEnter={e=>(e.currentTarget.style.borderColor=h2r(ACCENT,0.35))}
                  onMouseLeave={e=>(e.currentTarget.style.borderColor=S.border)}
                  onClick={()=>navigate('/applications')}
                >
                  <div>
                    <div style={{ fontSize:14, fontWeight:700, color:S.text, fontFamily:S.font, marginBottom:2 }}>{app.college_name}</div>
                ...
                {essayProgress.map((essay:any)=>{
                  ...
                  return (
                    <div key={essay.id} style={{ padding:'14px 16px', background:S.surface2, border:`1px solid ${h2r(color,0.25)}`, borderTop:`2px solid ${color}`, borderRadius:12 }}>
                      <div style={{ fontSize:14, fontWeight:700, color:S.text, marginBottom:4, fontFamily:S.font }}>{essay.college_name}</div>
